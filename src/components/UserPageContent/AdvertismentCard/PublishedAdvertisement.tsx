...
export const PublishedAdvertisement = () => {
    ...
    return(
        ...
        {accommodations && accommodations !== undefined ?
            <>
                {accommodations.map((accommodation) => (accommodation.status === 'published' && accommodation.user_id===Number(userId)) ? <ConfirmedItem props={accommodation}/> : <></>)}
            </>
            :
            <>
                <Typography fontSize={28} fontWeight={500} sx={{marginBottom: '35px'}}>На даний момент у вас немає опублікованих пропозицій</Typography>
