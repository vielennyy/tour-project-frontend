...
import logOut from '../../../assets/icons/UserPage/exitIcon.svg'
        ...
        <Box  
            ...
            >
                        <Link 
                            ...
                            sx={{marginRight: '10px'}}>
                            <Typography fontSize={22} fontWeight={400}>Вийти з акаунту <img src={logOut}/></Typography>
...
const PasswordBox = ({user, editPassword, setEditPassword, userToken}:PasswordBoxProps) => {
        ...
        <Box sx = {{boxShadow: '0px 4px 15px rgba(3, 2, 2, 0.25)',
            ...
            <Box display="flex" flexDirection={'row'} justifyContent={'space-between'}>
                <Typography fontSize={28} fontWeight={500}>Пароль</Typography>
                    <Box display="flex" flexDirection={'row'}>
                        <Link underline="hover" color="inherit" fontSize='25' onClick={()=>{setEditPassword(!editPassword)}} sx={{marginRight: '10px', cursor: 'pointer'}}>
                            <Typography fontSize={22} fontWeight={400}>Редагувати</Typography>
                        </Link>
                        <SvgIcon sx={{marginTop: '5px'}}><EditIcon/></SvgIcon>
                    </Box>
...
export const EditPasswordBox = ({user, userToken, editPassword, setEditPassword}:PasswordBoxProps) => {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        console.log(formState)
            if(formState.new_password === formState.confirm_password) {
                ...
                fetch(`https://cktour.club/api/v1/password/update`, {
                    method: "PUT",
                    headers: {
    ...
    return(
        <Box sx = {{boxShadow: '0px 4px 15px rgba(3, 2, 2, 0.25)',
            ...
            <Box fontSize={22} display="flex" flexDirection={'row'} alignItems={'center'} justifyContent={'space-between'} marginBottom={'30px'}>
                <Typography fontSize={28} fontWeight={500}>Пароль</Typography>
                <Box display="flex" flexDirection={'row'} >
                    <Link underline="hover" color="inherit" fontSize='25' onClick={() => {setEditPassword(false)}} sx={{marginRight: '10px', cursor: 'pointer'}}>
            ...
            <form onSubmit={(event: React.FormEvent<HTMLFormElement>) => handleSubmit(event)}>
                <FormControl sx={{display: 'flex', flexDirection: 'column'}}>
                    ...
                    <TextField name='old_password' type="password" onChange={handleFormChange} required/>
                    <Typography fontSize={18} marginTop={'20px'} marginBottom={'5px'}>Новий пароль</Typography>
                    <TextField name='new_password' type="password" onChange={handleFormChange} required/>
                    <Typography fontSize={18} marginTop={'20px'} marginBottom={'5px'}>Повторіть пароль</Typography>
                    <TextField name='confirm_password' type="password" onChange={handleFormChange} required/>
